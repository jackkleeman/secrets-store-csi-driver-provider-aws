...
func (s *CSIDriverProviderServer) Mount(ctx context.Context, req *v1alpha1.MountRequest) (response *v1alpha1.MountResponse, e error) {
	...
	awsSessions, err := s.getAwsSessions(nameSpace, svcAcct, podName, ctx, regions)
	if err != nil {
		return nil, err
	}
...
func (s *CSIDriverProviderServer) getAwsSessions(nameSpace, svcAcct, podName string, ctx context.Context, lookupRegionList []string) (response []*session.Session, err error) {
	podUID, hasPodIdentity, err := s.getPodInfo(ctx, nameSpace, podName)

	// Get the pod's AWS creds for each lookup region.
	var awsSessionsList []*session.Session

	for _, region := range lookupRegionList {
		oidcAuth, err := auth.NewAuth(ctx, region, nameSpace, svcAcct, podName, podUID, hasPodIdentity, s.k8sClient)
		if err != nil {
...
func (s *CSIDriverProviderServer) getPodInfo(ctx context.Context, namespace, podName string) (string, bool, error) {
	...
	for _, v := range pod.Spec.Volumes {
		...
		for _, s := range v.Projected.Sources {
			...
			if s.ServiceAccountToken.Audience == "pods.eks.amazonaws.com" {
